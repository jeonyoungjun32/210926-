...
<section id="listForm">
	...
	<c:if test="${iceList != null}"> <!-- 1.개 상품목록이 있으면 -->
		<h2>아이스크림 상품 목록 </h2>
		<table>
			<tr> <!-- var:반복할 변수이름, items: 반복할 객체 이름, 
			          varStatus: 반복상태 속성 (status.index:0부터 시작)-->
			<c:forEach var="ice" items="${iceList}" varStatus="status"> <!-- 향상된 for -->
				<td>
					<a href="iceView.ice?id=${ice.id}"><img src="images/${ice.image}" id="productImage" /></a>
					상품명: ${ice.kind}<br/>
					가격 :${ice.price}<br/>
				</td>
	...
	<c:if test="${iceList == null}"> <!-- 2.개 상품목록이 없으면 -->
		<div class="div_empty">개상품이 없습니다. 분양불가</div>
	</c:if>
	
	<!-- 3.쿠키객체들(오늘 본 개 상품 이미지)에서 얻어온 "이미지 파일이름들"을 추가한 todayImageList -->
	<c:if test="${(todayImageList != null) && (iceList != null) }"> <!--개상품목록에서 "개이미지"를 클릭하면 -->	
		<div id="todayImageList">
			<h2>오늘 본 아이스크림 상품 목록</h2>
			<table>
				<tr>
